  ...
  <v-layout align-start>
    <v-flex>
      <v-toolbar flat color="white">
        <v-toolbar-title>Promocion</v-toolbar-title>
      ...
      >
        <template v-slot:items="props">
          ...
          <td>{{ props.item.tipopromocion }}</td>
          <td>{{ props.item.promocion }}</td>
          <td>{{ formatearFecha(props.item.fechainicio) }}</td>
          <td>{{formatearFecha( props.item.fechafin) }}</td>
        ...
        <v-layout row wrap>
          ...
          <v-flex xs12 sm4 md4 lg4>
                    <v-dialog
                      ref="dialog1"
                      v-model="modal3"
                      :return-value.sync="inicioPromocion"
                      persistent
                      width="290px"
                    >
                      <template v-slot:activator="{ on, attrs }">
                        <v-text-field
                          v-model="inicioPromocion"
                      ...
                      <v-date-picker v-model="inicioPromocion" scrollable>
                        <v-spacer></v-spacer>
                        <v-btn text color="primary" @click="modal3 = false">Cancelar</v-btn>
                        <v-btn
                          text
                          color="success"
                          @click="$refs.dialog1.save(inicioPromocion)"
                        >Aceptar</v-btn>
                      </v-date-picker>
                    </v-dialog>
          </v-flex>
          ...
          <v-flex xs12 sm4 md4 lg4>
                    <v-dialog
                      ref="dialog"
                      v-model="modal2"
                      :return-value.sync="finpromocion"
                      persistent
                      width="290px"
                    >
                      <template v-slot:activator="{ on, attrs }">
                        <v-text-field
                          v-model="finpromocion"
                      ...
                      <v-date-picker v-model="finpromocion" scrollable>
                        <v-spacer></v-spacer>
                        <v-btn text color="primary" @click="modal2 = false">Cancelar</v-btn>
                        <v-btn
                          text
                          color="success"
                          @click="$refs.dialog.save(finpromocion)"
                        >Aceptar</v-btn>
                      </v-date-picker>
                    </v-dialog>
          </v-flex>
          <v-flex xs12 sm6 md6 lg6 xl6>
            <v-text-field v-model="promocion" label="Promocion" ></v-text-field>
          </v-flex>
            ...
            <template>
              ...
              >
                <template slot="items"  slot-scope="props">
                  ...
                  <td class="text-xsr blue--text">{{ props.item.codigoB }}</td>
                  <td class="text-xsr blue--text">{{ props.item.producto }}</td>
                  <td class="text-xsr blue--text">{{ props.item.codigoInventario.descripcion }}</td>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Tipo promocion", value: "tipopromocion", sortable: false },
        { text: "Promocion", value: "promocion", sortable: false },
      ...
      tiposcomprobantes: [ 
          { text: "BONIFICACION", value: "bonificacion"},
      ...
      cabeceraDetalles: [
        { text: "Borrar", value: "borrar", sortable: false },
         { text: "Codigo de Barras", value: "codigoB", sortable: false },
      ...
      cabeceraArticulos: [
        ...
        { text: "Farmacia", value: "codigoInventario.descripcion", sortable: false },
        { text: "Estado", value: "estado", sortable: false },
      ],
      verDetalle: 0,
      valida: 0,
    ...
    calcularTotal: function () {
      ...
      for (var i = 0; i < this.detalles.length; i++) {
        if (this.detalles[i].iva == 1) {
          resultado =
            resultado +
            this.detalles[i].cantidad * this.detalles[i].costoNeto * 0.12 +
            this.detalles[i].cantidad * this.detalles[i].costoNeto;
        } else {
          resultado =
  ...
  methods: {
    Eliminar(data){
      ...
      if (codigoFarmacia == undefined) {
        axios
        .delete("promocion/remove?_id="+data._id, configuracion)
        .then(function (response) {
          if(response.status===200){
            Swal.fire('Notificación','Se elimino la promocion!','success')
           me.listar();
          }else{
    ...
    listarDetalles(id) {
      ...
      let configuracion = { headers: header };
      axios
        .get("promocion/query?_id=" + id, configuracion)
        .then(function (response) {
          me.detalles = response.data.producto;
    ...
    verIngreso(item) {
      ...
      this.codigoTipoComprobante=item.tipopromocion
      this.promocion=item.promocion
      this.inicioPromocion=this.formatearFecha(item.fechainicio)
      this.finpromocion=this.formatearFecha(item.fechafin)
    ...
    listar() {
      ...
      if (codigoFarmacia == undefined) {
        axios
          .get("promocion/list", configuracion)
          .then(function (response) {
    ...
    validar() {
      ...
      var Ainicio = this.inicioPromocion.slice(0,4)
      
      var Minicio = this.inicioPromocion.slice(5,7)
     
      var Dinicio = this.inicioPromocion.slice(8,10)
      
      
      var Afin = this.finpromocion.slice(0,4)
      
      var Mfin = this.finpromocion.slice(5,7)
    
      var Dfin = this.finpromocion.slice(8,10)
      if(parseInt(Ainicio)<=parseInt(Afin) && parseInt(Minicio)<=parseInt(Mfin) && parseInt(Dinicio)<=parseInt(Dfin)){
         
      }else{
        this.validaMensaje.push("La fecha de fin de promocion debe ser mayor que la de inicio.");
      }
      if(!this.codigoTipoComprobante) {
        this.validaMensaje.push("Elija un tipo de promocion");
      }
      if(!this.promocion) {
        this.validaMensaje.push("Ingrese la promocion, (Ej:1+1 / 50%)");
      }
      if(!this.inicioPromocion) {
        this.validaMensaje.push("Debe ingresar una fecha de incio.");
      }
      if (!this.finpromocion) {
        this.validaMensaje.push("Debe ingresar una fecha de fin.");
      }
      if (!this.detalles.length) {
        this.validaMensaje.push("Al menos un producto debe agregar..");
      }
      ...
      axios
        .post(
          "promocion/add",
          {
            ...
            fechainicio: this.inicioPromocion,
            fechafin: this.finpromocion,
            tipopromocion:this.codigoTipoComprobante,
    ...
    filtrarFechas() {
      ...
      if (codigoFarmacia == undefined) {
        axios
          .get(
            "promocion/listporfechasad?fechainicio=" +
              this.finicio +
              "&fechafin=" +
              this.ffin +
              ""
    ...
    crearPDF() {
      ...
      var columns = [
        { title: "Tipo promocion", dataKey: "tipopromocion" },
        { title: "Promocion", dataKey: "promocion" },
      ...
      doc.autoTable(columns, rows, {
        ...
        addPageContent: function (data) {
          doc.text("Reporte de promociones", 80, 20);
      ...
      doc.save("ReportePromocioness.pdf");
    },
  },
};
</script>
