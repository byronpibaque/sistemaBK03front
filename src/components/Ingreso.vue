  ...
  <v-layout align-start>
    <v-flex>
      <v-toolbar flat color="white">
       ...
       <v-btn class="success" @click="mostrarExportar">
          Fechas
        </v-btn>
          <v-btn class="primary" @click="mostrarFarmacias"  v-if="$store.state.usuario.codigoFarmacia == undefined ">
          Farmacias
        </v-btn>
        ...
        ></v-text-field>
        <v-spacer></v-spacer>
        <v-btn color="primary" v-if="verNuevo==0 && $store.state.usuario.codigoFarmacia != undefined " @click="mostrarNuevo()" dark class="mb-2">Nuevo</v-btn>
        <v-dialog v-model="farmaciaModal" max-width="500px">
          <v-card>
            <v-card-title>
              <span class="headline"
            ...
            <v-card-actions>
              ...
              <v-btn color="secundary" @click="cerrarFarmacias()"
                >Cancelar</v-btn
              >
            </v-card-actions>
          </v-card>
      ...
      >
        <template v-slot:items="props">
          <td class="justify-center layout px-0">
            ...
            <v-icon small class="mr-2" @click="imprimirTicket(props.item)">print</v-icon>
              ...
              >
                <template slot="items" slot-scope="props">
                  ...
                  <td class="text-xs-center">
                   ...
                   <td class="text-xs-center">
                    <v-text-field v-model="props.item.fraccionesN" ></v-text-field>
                  </td>
                  <td class="text-xs-center green--text">
                  {{ props.item.fracciones}}
                  </td>
                  <td class="text-xs-center">
                  <span class="blue--text">C:</span>{{props.item.cantidad}}-<span class="blue--text">f:</span>{{props.item.fraccionesN}}
                  </td>
                  <td class="text-xs-center">
                    <v-text-field v-model="props.item.costoNeto" ></v-text-field>
                  </td>
                   <td>
                  ...
                  <td class="text-xs-right">
                              ...
                              {{ (props.item.cantidad * props.item.costoNeto)+(props.item.fraccionesN*(props.item.costoNeto/props.item.fracciones).toFixed(2))  }}
...
export default {
    ...
    return {
      ...
      cabeceraDetalles: [
        ...
        {text:"Fracciones",value:"fraccionesN",sortable:false},
         {text:"F * Caja",value:"fracc",sortable:false},
         {text:"Ingreso",value:"fracc",sortable:false},
  ...
  computed: {
    calcularTotal: function() {
      let resultado = 0.0;
          resultado = resultado + this.caltularSubtotal+this.calcularIva   
      return resultado;
    },
    caltularSubtotal: function() {
      ...
      for (var i = 0; i < this.detalles.length; i++) {
        ...
        resultado =
          resultado + (this.detalles[i].cantidad * this.detalles[i].costoNeto)+(this.detalles[i].fraccionesN*(this.detalles[i].costoNeto/this.detalles[i].fracciones).toFixed(2));
  ...
  methods: {
    filtrarFarmacias() {
      ...
      if (codigoFarmacia == undefined) {
        axios
          .get(
            "ingresos/listfiltrado?valor=" + me.codigoFarmacia,
            configuracion
          )
    ...
    mostrarFarmacias () { 
      this.farmaciaModal=1
    },
    cerrarFarmacias(){
      this.farmaciaModal=0
    },
    encabezadosXML (emite,usuario,descripcion,numeroComprobante,total) {
      ...
      axios
        .post(
          "http://localhost:25000/api/xml/xmlIngreso",
          {
            ingreso: {
              farmacia: emite,
    ...
    imprimirTicket (item) { 
     this.detalleIngreso=[]
      this.detalleIngreso.push(item.detalles)
      this.encabezadosXML(
    ...
    selectFarmacias() {
      ...
      axios
        .get("farmacia/query?_id="+codigoFarmacia, configuracion)
        .then(function(response) {
           me.farmacias.push({ text: response.data.descripcion, value: response.data._id })
      ...
      axios
        ...
        .then(function(response) {
          ...
          tipocomprobanteArray.map(function(x) {
            ...
            me.codigoTipoComprobante=x._id
          });
        })
        .catch(function(error) {
          console.log(error);
    ...
    listar() {
      ...
      if (codigoFarmacia == undefined) {
       ...
       axios
              .get(
                "ingresos/list?valor=" + codigoFarmacia,
                configuracion
              )
              .then(function(response) {
               me.ingresos = response.data;
    ...
    validar() {
      ...
      for (let x = 0; x < this.detalles.length; x++) {
        ...
        const element = this.detalles[x];
        if(element.fraccionesTotales==null ||element.fraccionesTotales<0){
            this.validaMensaje.push("El producto "+element.producto+" tiene inconsistencias en fracciones totales. Revise.");
        }
        if(element.fracciones==null || element.fracciones==0){
            this.validaMensaje.push("El producto "+element.producto+" tiene inconsistencias en el fraccionamiento de la caja. Revise");
        }
         if(parseInt(this.detalles[x].cantidad)==0 ||   parseInt(this.detalles[x].cantidad)>0){
          ...
          if(parseInt(this.detalles[x].fraccionesN)==0 ||   parseInt(this.detalles[x].fraccionesN)>0){
          ...
          if (this.detalles[x].cantidad == 0) {
            if (this.detalles[x].fraccionesN > 0) {
     ...
     this.banderabtnGuardar=1
      ...
      axios
        .post(
          "ingresos/add",
          {
            codgioPersona: this.codgioPersona,
            codigoUsuario: this.$store.state.usuario.codigoUsuario,
            codigoFarmacia:this.$store.state.usuario.codigoFarmacia,
            codigoTipoComprobante: this.codigoTipoComprobante,
            serieComprobante: "001",
        ...
        .then(function(response) {
          if(response.status==200){
            Swal.fire("Noticias!","Se guardo correctamente el ingreso, verifique Stock.","success")
          }else{
    ...
    validarF() {
      ...
      if(parseInt(Ainicio)<=parseInt(Afin) && parseInt(Minicio)<=parseInt(Mfin) && parseInt(Dinicio)<=parseInt(Dfin)){
    ...
    filtrarFechas(){
     ...
     if (codigoFarmacia == undefined) {
        axios
          .get("ingresos/listporfechasad?fechainicio="+fechainicial.toISOString()+"&fechafin="+fechafinal.toISOString())
         .then(function(response) {
          me.ingresos = response.data;
          me.ocultarExportar()
         })
    ...
    crearPDF() {
      ...
      var columns = [
        ...
        { title: "Proveedor", dataKey:"codgioPersona"},
        { title: "Descripcion", dataKey:"descripcion" },
        { title: "Tipo Comprobante",dataKey:"codigoTipoComprobante"},
      ...
      this.ingresos.map(function(x) {
        rows.push({
          ...
          codgioPersona: x.codgioPersona.nombres,
          descripcion: x.descripcion,
          codigoTipoComprobante: x.codigoTipoComprobante.descripcion,
