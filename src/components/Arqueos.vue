  ...
  <v-layout align-start>
    <v-flex>
      <v-toolbar flat color="white">
        <v-toolbar-title>Arqueos</v-toolbar-title>
        ...
        ></v-text-field>
        <v-spacer></v-spacer>
        <v-dialog v-model="ArqueosModal" max-width="450px">
          <v-card>
            <v-card-title>
              <span class="headline">Ver total de caja hasta el momento</span>
            </v-card-title>
            <v-card-text >
                 <v-container grid-list-md>
                <v-layout wrap>
                   <v-flex xs12 sm12 md12 lg12 xl12>
                              <v-autocomplete :items="farmacia" v-model="farma" v-on:change="selectUsuariosF()" label="Farmacia:"></v-autocomplete>
                            </v-flex>
                             <v-flex xs12 sm12 md12 lg12 xl12>
                              <v-autocomplete :items="usuarios" v-model="codigoUser" label="Usuario:"></v-autocomplete>
                            </v-flex>
                  <v-btn color="info" @click="totalVentaArq()">Calcular</v-btn>
                <v-flex xs12 sm12 md12 lg12 xl12  class="font-weight-bold green--text" style=" font-size:35px" v-if="farma!='undefined'">
                        TOTAL:${{totalactual}}
                </v-flex>
                <v-flex xs12 sm12 md12 lg12 xl12  class="font-weight-bold blue--text" style=" font-size:20px" v-if="farma!='undefined'">
                        TOTAL EFECTIVO:${{EFES}}
                </v-flex>
                  <v-flex xs12 sm12 md12 lg12 xl12  class="font-weight-bold blue--text" style="font-size:20px" v-if="farma!='undefined'">
                        TOTAL T. CRÉDITO:${{TCS}}
            ...
            <v-card-actions class="center">
              <v-btn color="success" @click="ocultarArqueos()">Cerrar</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
                  ...
                  <v-flex xs12 sm6 md6>
                    ...
                    >
                      ...
                      <v-date-picker v-model="finicio" scrollable>
                        ...
                        <v-btn text color="success" @click="$refs.dialog1.save(finicio)">Aceptar</v-btn>
                      </v-date-picker>
                    </v-dialog>
                  </v-flex>
                  <v-flex xs12 sm6 md6>
                    ...
                    >
                      ...
                      <v-date-picker v-model="ffin" scrollable>
                        ...
                        <v-btn text color="success" @click="$refs.dialog.save(ffin)">Aceptar</v-btn>
                      </v-date-picker>
                    </v-dialog>
                  </v-flex>
                  
                  <v-flex  xs12 sm12 md12 lg12 v-if="$store.state.usuario.codigoFarmacia == undefined">
                      <v-select v-model="codigoFar" :items="farmacia" @change="listar()"  label="Farmacia"></v-select>
                    </v-flex>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          <template v-slot:activator="{ on }" v-if="bandera==false">
            <v-btn class="success" @click="mostrarArqueos" v-if="$store.state.usuario.codigoFarmacia == undefined">Ver  <v-icon>visibility</v-icon></v-btn>
            <v-btn id="BtnNuevo" color="primary" dark class="mb-2" v-on="on">Nuevo <v-icon>add</v-icon></v-btn>
          </template>
          <v-card>
            <v-card-title>
              ...
              <v-container grid-list-md>
                <v-layout wrap>
                  <v-flex xs12 sm12 md12>
                    <v-text-field v-model="totalCaja" label="Total caja:"></v-text-field>
                  </v-flex>
                  <v-flex xs12 sm12 md12 v-show="valida">
                    <div class="red--text" v-for="v in validaMensaje" :key="v" v-text="v"></div>
                  </v-flex>
      ...
      <v-dialog v-model="desgloceD" max-width="450px">
          <v-card>
       
            <v-card-title class="headline">Desgloce</v-card-title>
            <v-card-text>
                <table class="text-xs-center">
                <tr>
                 <td  class="font-weight-bold blue--text" style=" font-size:15px">Total Caja:</td><span class="font-weight-bold " style=" font-size:15px">${{D_Caja}}</span>
                </tr>
                 <tr>
                 <td  class="font-weight-bold blue--text" style=" font-size:15px">Total BK03:</td><span class="font-weight-bold " style=" font-size:15px">${{D_BK}}</span>
                </tr>
                <tr>

                    <td  class="font-weight-bold blue--text" style=" font-size:15px">Total Efectivo</td>
                    <td  class="font-weight-bold blue--text" style=" font-size:15px">Total Credito</td>
                    <td  class="font-weight-bold blue--text" style=" font-size:15px">Total T. Debito</td>
                    <td  class="font-weight-bold blue--text" style=" font-size:15px">Total T. Credito</td>

                </tr>

                <tr>

                    <td  class="font-weight-bold" style=" font-size:15px">${{D_EFES}}</td>
                    <td  class="font-weight-bold" style=" font-size:15px">${{D_CRES}}</td>
                    <td  class="font-weight-bold" style=" font-size:15px">${{D_TBS}}</td>
                    <td  class="font-weight-bold" style=" font-size:15px">${{D_TCS}}</td>

                </tr>
                 <tr>
                 <td  class="font-weight-bold blue--text" style=" font-size:15px">Positivos:</td><span class="font-weight-bold green--text " style=" font-size:15px">${{D_Pos}}</span>
                </tr>
                 <tr>
                 <td  class="font-weight-bold blue--text" style=" font-size:15px">Negativos:</td><span class="font-weight-bold red--text " style=" font-size:15px">${{D_Neg}}</span>
                </tr>
              </table>
            </v-card-text>
            <v-card-actions>
        ...
        <template v-slot:items="props">
          ...
          <td class="justify-center layout px-0">
            <v-icon small class="mr-2" @click="editItem(props.item)">edit</v-icon>
              <v-icon small @click="eliminarRegistro(props.item)">delete</v-icon>
              <v-icon small @click="verDesgloce(props.item)">tab</v-icon>
          ...
          <td
            v-if=" props.item.totalCaja ==  props.item.totalBK03"
            class="green--text"
          >{{ props.item.codigoFarmacia.descripcion }}</td>
          <td
            v-else-if="props.item.totalCaja <=  props.item.totalBK03"
            class="red--text"
          >{{ props.item.codigoFarmacia.descripcion }}</td>
          <td
            v-else-if=" props.item.totalCaja >=  props.item.totalBK03"
          ...
          <td
            v-if=" props.item.totalCaja ==  props.item.totalBK03"
            class="green--text"
          >{{ props.item.codigoUsuario.nombres }}</td>
          <td
            v-else-if="props.item.totalCaja <=  props.item.totalBK03"
            class="red--text"
          >{{ props.item.codigoUsuario.nombres }}</td>
          ...
          <td
            v-if=" props.item.totalCaja ==  props.item.totalBK03"
            class="green--text"
          >${{ props.item.totalCaja.toFixed(2) }}</td>
          <td
            v-else-if="props.item.totalCaja <=  props.item.totalBK03"
            class="red--text"
          >${{ props.item.totalCaja.toFixed(2)}}</td>
          ...
          <td
            v-if=" props.item.totalCaja ==  props.item.totalBK03"
            class="green--text"
          >${{ props.item.totalBK03.toFixed(2) }}</td>
          <td
            v-else-if="props.item.totalCaja <=  props.item.totalBK03"
            class="red--text"
          >${{ props.item.totalBK03.toFixed(2)}}</td>
          <td
            ...
            class="orange--text"
          >${{ props.item.totalBK03.toFixed(2) }}</td>
             <td class="green--text">${{ props.item.positivos.toFixed(2) }}</td>
             <td class="green--text">${{ props.item.negativos.toFixed(2) }}</td>
          <td>
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Seccion", value: "seccion", sortable: true },
        { text: "Fecha", value: "createdAt", sortable: true },
        { text: "Total caja", value: "totalCaja", sortable: false },
        { text: "Total sistema", value: "totalBK03", sortable: false },
         { text: "Positivos", value: "positivos", sortable: false },
          { text: "Negativos", value: "negativos", sortable: false },
  ...
  methods: {
    encabezadosXML (emite,usuario,totalSistema,totalCaja,positivos,negativos,desgloce) {
      ...
      axios
        .post(
          "http://localhost:25000/api/xml/xmlArqueos",
          {
            arqueo: {
              farmaciaEmite: emite,
              usuario: usuario,
    ...
    imprimirTicket (item) { 
      ...
      this.encabezadosXML(
        item.codigoFarmacia.descripcion,
        item.codigoUsuario.nombres,
        item.totalBK03.toFixed(2),
        item.totalCaja.toFixed(2),
        item.positivos.toFixed(2),
        item.negativos.toFixed(2),
        item.desgloce
        )   
    ...
    verDesgloce(item){
      this.desgloceD=1
        this.D_Neg=item.negativos.toFixed(2)
        this.D_Pos=item.positivos.toFixed(2)
        this.D_Caja=item.totalCaja.toFixed(2)
        this.D_BK=item.totalBK03.toFixed(2)
      item.desgloce.forEach(element => {
        this.D_EFES=element.ef.toFixed(2)
        this.D_CRES=element.cr.toFixed(2)
        this.D_TBS=element.td.toFixed(2)
        this.D_TCS=element.tc.toFixed(2)
      });
    ...
    selectUsuariosF() {
      ...
      axios
        .get("login/listFiltro?codigoFarmacia=" + this.farma, configuracion)
    ...
    ObtenerTurnos(){
      ...
      let configuracion = { headers: header };
       if(codigoFarmacia==undefined){
         axios.get('farmacia/query?_id='+me.farma,configuracion)
              .then(function (response){
                me.horarioAPT=response.data.horarioAPT
                me.horarioCPT=response.data.horarioCPT
                me.horarioAST=response.data.horarioAST
                me.horarioCST=response.data.horarioCST
              }).catch(function(error){
                  console.log(error);
              });
      ...
      let configuracion = { headers: header };
        if(codigoFarmacia==undefined){
          ...
          let fechaConverI = new Date(data+" 00:00:01");
             let fechaConverF = new Date(fechaFinal + " "+tiempo);

                axios
                  .get(
                    "ventas/reporteV?start="+   fechaConverI.toISOString() +"&end="+  fechaConverF.toISOString()+"&codigoFarmacia="+me.farma+"&codigoUsuario="+me.codigoUser,
                    configuracion
                  )
                  .then(function (response) {
                    ...
                    response.data.forEach(element => {
                        ...
                        if(element.formapago=="EFECTIVO")
                          {
                            Efsuma+=parseFloat(element.total) 
                          
                           
                          }else if(element.formapago=="TARJETA DE CRÉDITO"){
                            Tcsuma+=parseFloat(element.total) 
                            
                          }else if(element.formapago=="TARJETA DE DÉBITO"){
                            Tdsuma+=parseFloat(element.total) 
                            
                          }else if(element.formapago=="CRÉDITO"){
                            Cresuma+=parseFloat(element.total) 
                           
                          }
                           me.EFES=Efsuma.toFixed(2)
                           me.TCS=Tcsuma.toFixed(2)
                           me.TBS=Tdsuma.toFixed(2)
                           me.CRES=Cresuma.toFixed(2)
                      totalSuma=parseFloat(Efsuma)+parseFloat(Tcsuma)+parseFloat(Tdsuma)
                      me.totalactual=totalSuma.toFixed(2)  
                    });
    ...
    VerificarArqueo() {
      ...
      if (codigoFarmacia != undefined) {
            ...
            let header = { Token: me.$store.state.token };
              let configuracion = { headers: header };
                  var FECHA = new Date(data+" 00:00:01");
                  var FECHA2 = new Date(data+" 23:59:59");
                axios
              .get(
                "arqueo/existe?cf=" +
                  codigoFarmacia +
              ...
              .then(function (response) {
                ...
                if(response.data.length==0){
                    Swal.fire(
                            "Recuerde",
                            "Solo puede ingresar un arqueo de caja.",
                            "warning"
                          );
                }
                else{
                  response.data.forEach(element => {
                    if(data==me.formatearFechaT(element.createdAt)){
                          me.bandera = true;
                            Swal.fire(
                              "Notificación",
                              "Ya hizo el arqueo de caja, Ya no puede realizar otro.",
                              "error"
                            ); 
                        }else{
                          Swal.fire(
      ...
      Swal.fire("Notificacion", "Ha ingresado como administrador", "success");
      }
    },
           ...
           let header = { Token: me.$store.state.token };
            let configuracion = { headers: header };
             var FECHA = new Date(data+" 00:00:01");
              ...
              axios
              .get(
                    "ventas/reporteV?start="+   FECHA.toISOString() +"&end="+  FECHA2.toISOString()+"&codigoFarmacia="+me.$store.state.usuario.codigoFarmacia+"&codigoUsuario="+me.$store.state.usuario.codigoUsuario,configuracion,
                    configuracion
                  )
              .then(function (response) {
                    let Efsuma=0.0
                        ...
                        if(element.formapago=="EFECTIVO")
                          {
                            ...
                            me.desgloce={
                                tc:Tcsuma.toFixed(2), 
                                td:Tdsuma.toFixed(2), 
                                cr:Cresuma.toFixed(2), 
                                ef:Efsuma.toFixed(2),
                            }
                             me.totalBK03 = Efsuma.toFixed(2)
                    });
              })
              .catch(function (error) {
                console.log(error);
    ...
    listar() {
      ...
      // prints date in YYYY-MM-DD format
      let data = year + "-" + month + "-" + date;
      let Fi = new Date(data+" 00:00:01")
      let Ff = new Date(data+" 23:59:59")
     
      let header = { Token: this.$store.state.token };
      let configuracion = { headers: header };
      if (codigoFarmacia != undefined) {
         axios
          .get("arqueo/list?codigoFarmacia="+codigoFarmacia+"&Finicio="+Fi.toISOString()+"&Ffin="+Ff.toISOString(), configuracion)
          .then(function (response) {
            me.arqueo = response.data;
          })
          .catch(function (error) {
            console.log(error);
          });
    ...
    validar() {
      ...
      this.validaMensaje = [];
      if (!this.totalCaja) {
        this.validaMensaje.push("Debe ingresar el total de la caja.");
      }
      if(parseFloat(this.totalBK03)==0){
        this.validaMensaje.push("No hay ventas registradas en el sistema.");
      }
    ...
    guardar() {
      ...
      this.codigoFarmacia = this.$store.state.usuario.codigoFarmacia;
      this.codigoUsuario = this.$store.state.usuario.codigoUsuario;

      let totalBK=(parseFloat(this.totalBK03)).toFixed(2)
      let totalCaja=(parseFloat(this.totalCaja)).toFixed(2)
      let sobrante = parseFloat(totalCaja)-parseFloat(totalBK)
      let positivos=0.0
      let negativos = 0.0
      if(parseFloat(totalBK)==parseFloat(totalCaja)){//TOTAL DE SISTEMA IUGAL A TOTAL DE CAJA
        positivos=0
        negativos=0
      }else if(parseFloat(totalBK)>=parseFloat(totalCaja)){//TOTAL DE SISTEMA MAYOR QUE TOTAL DE CAJA
        positivos=0
        negativos=sobrante
      }else if(parseFloat(totalBK)<=parseFloat(totalCaja)){//TOTAL DE SISTEMA MENOR QUE TOTAL DE CAJA
        positivos=sobrante
        negativos=0
      }
      ...
      me.loading = true;
         ...
         axios.post("arqueo/add", {
          ...
          codigoFarmacia: me.codigoFarmacia,
          seccion:"GLOBAL",
          positivos:positivos,
          negativos:negativos,
    ...
    activar() {
      let me = this;
      axios
        .put("arqueo/activate", { _id: this.adId })
        .then(function (response) {
          me.adModal = 0;
          me.adAccion = 0;
          me.adNombre = "";
    ...
    desactivar() {
      let me = this;
      axios
        .put("arqueo/deactivate", { _id: this.adId })
        .then(function (response) {
          me.adModal = 0;
          me.adAccion = 0;
          me.adNombre = "";
    ...
    filtrarFechas() {
      ...
      if (codigoFarmacia == undefined) {
        axios
          .get(
            "arqueo/listporfechas?fechainicio=" +
              fechaConverI.toISOString() +
              "&fechafin=" +
              fechaConverF.toISOString() +
              "&codigoFarmacia="+this.codigoFar
          )
          .then(function (response) {
            me.arqueo = response.data;
            me.ocultarExportar();
    ...
    crearPDF() {
      ...
      var columns = [
        { title: "Farmacia", dataKey: "codigoFarmacia" },
        { title: "Usuario", dataKey: "codigoUsuario" },
        { title: "Seccion", dataKey: "seccion" },
        { title: "Fecha", dataKey: "createdAt" },
        { title: "Total caja", dataKey: "totalCaja" },
        { title: "Total sistema", dataKey: "totalBK03" },
      ];
      /*   headers: [
                   
                    { text: 'Farmacia', value: 'codigoFarmacia.descripcion', sortable: true },
                    { text: 'Usuario', value: 'codigoUsuario.nombres', sortable: false },
                    { text: 'Seccion', value: 'seccion', sortable: true },
      ...
      doc.autoTable(columns, rows, {
        ...
        addPageContent: function (data) {
          doc.text("Reporte de arqueos", 80, 20);
          doc.setTextColor("blue");
          doc.text("LINEA DE FARMACIAS TU AHORRO", 60, 30);
          doc.text("Administrador", 85, 40);
          doc.addImage(img, "JPEG", 160, 10, 35, 35);
        },
      });
      doc.save("ReporteArqueos.pdf");
    },
  },
};
</script>
