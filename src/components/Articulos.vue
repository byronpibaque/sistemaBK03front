  ...
  <v-layout align-start>
    <v-flex>
      <v-toolbar flat color="white">
        <v-btn @click="abrirModalExportar">
          EXCEL
          <v-icon>save</v-icon>
        </v-btn>
        ...
        ></v-text-field>
        <v-spacer></v-spacer>
        <v-dialog v-model="exportarExcel" max-width="500px">
          <v-card>
            <v-card-title>
            ...
            <v-card-text>
              <v-flex xs12 sm12 md12 ld12 xl12>
                ...
                ></v-autocomplete>
                 ...
                 <v-flex xs5 sm5 md5 xl5>
                  <v-checkbox 
                      style="margin-top:15px" 
                      color="success"
                      v-model="chckTotalizado"
                      label="Exportar inventario total."
                  ></v-checkbox>
                </v-flex>
            </v-card-text>
            <v-card-actions>
              <v-btn color="blue darken-1" flat @click="exportarExcel=0"
                >CERRAR</v-btn>
                <v-btn color="green darken-1" flat @click="exportarInventario"
                  >EXPORTAR</v-btn>
            </v-card-actions>
          </v-card>
        </v-dialog>
        <v-dialog v-model="verDuplicados" max-width="800px">
          <v-card>
            <v-card-title>
              <span class="headline">Productos duplicados</span>
              <v-spacer></v-spacer>
               <v-text-field
                  class="text-xs-center"
                  v-model="busqueporfis"
                  append-icon="search"
                  label="Búsqueda"
                  single-line
                  hide-details
            ...
            <v-card-text>
               ...
               <v-data-table
                  :headers="headersduplicados"
                  :items="articulosduplicados"
                   :search="busqueporfis"
                  class="elevation-1"
                >
                  <template v-slot:items="props">
                    ...
                    <td class="blue--text">{{ props.item.nombre }}</td>
            ...
            <v-card-actions>
               <v-btn color="blue darken-1" flat @click="cerrarDuplicados"
                >Cerrar</v-btn>

            </v-card-actions>
          </v-card>
        </v-dialog>
        <v-dialog v-model="modalInventario" max-width="500px">
          <v-card>
            <v-card-title class="headline"
              >Elija la letra a imprimir:
              <hr />
            </v-card-title>
            <v-card-text>
              <v-container grid-list-md>
                <v-layout wrap>
                  ...
                  <v-btn color="success" @click="cargarInventario"
                    >CONSULTAR</v-btn
                  >
                </v-layout>
                <v-btn color="primary" @click="imprimirAlfabetico"
                  >IMPRIMIR</v-btn
                >
              </v-container>
            </v-card-text>
            <v-card-actions>
              ...
              <v-btn color="blue darken-1" flat @click="cerrarInventario"
                >CERRAR</v-btn
              >
            </v-card-actions>
          </v-card>
        </v-dialog>
        <v-dialog v-model="abrirC" max-width="700px">
          <v-card>
            <v-card-title class="headline"
              >Elija los inventarios a clonar:
              <hr />
            </v-card-title>
            <v-card-text>
              <v-container grid-list-md>
                <v-layout wrap>
                  ...
                  <v-flex xs12 sm6 md6>
                    <v-select
                      v-model="codigoInventarioB"
                      :items="inventario"
                      label="Inventario B:"
                    ></v-select>
                  </v-flex>
                  <v-btn color="primary" @click="Clonar">CLONAR</v-btn>
                </v-layout>
              </v-container>
            </v-card-text>
            <v-card-actions>
              ...
              <v-btn color="blue darken-1" flat @click="cerrarClonar"
                >CERRAR</v-btn
              >
            </v-card-actions>
          </v-card>
        </v-dialog>
        <v-dialog v-model="abrirE" max-width="700px">
          <v-card>
            <v-card-title class="headline"
              >Elija el inventario a eliminar:
              <hr />
            </v-card-title>
            <v-card-text>
              <v-container grid-list-md>
                <v-layout wrap>
                  ...
                  <v-btn color="primary" @click="EliminarInv">ELIMINAR</v-btn>
                </v-layout>
              </v-container>
            </v-card-text>
            <v-card-actions>
              ...
              <v-btn color="blue darken-1" flat @click="cerrarEliminar"
                >CERRAR</v-btn
              >
            </v-card-actions>
          </v-card>
            ...
            <v-card-text>
              <v-container grid-list-md>
                <v-layout wrap>
                  ...
                  <v-flex xs12 sm6 md6 v-if="banderaexiste!=true">
                    <v-select
                      v-model="iva"
                      :items="ivade"
                      label="Iva"
                  ...
                  <v-flex xs12 sm12 md12  v-if="banderaexiste!=true">
                    <v-text-field
                      v-model="nombre"
                  ...
                  <v-flex xs12 sm12 md12 ld12 xl12  v-if="banderaexiste!=true && $store.state.usuario.codigoFarmacia==undefined">
                  <v-checkbox
                    style="margin-top:15px"
                    color="success"
                    v-model="chcktodoi"
                  ...
                  <v-flex xs12 sm12 md12  v-if="banderaexiste!=true && chcktodoi==false">
                    <v-autocomplete
                      v-model="codigoInventario"
                      :items="inventario"
                      label="Inventario"
                 ...
                 <v-flex
                  ...
                  <v-flex xs12 sm12 md12  v-if="banderaexiste!=true">
                    <div class="text-center">
                      <v-chip class="ma-2" color="success">Precios</v-chip>
                    </div>
                  </v-flex>
      ...
      >
         ...
         <v-btn color="blue" @click="abrirDuplicados">
          <span class="white--text">DUPLICADOS</span>
        </v-btn>
      </v-flex>
      <v-flex
        xs12
        ...
        v-if="$store.state.usuario.codigoFarmacia == undefined"
      >
        <v-btn color="success" @click="abrirClonar">
          <v-icon>copy_all</v-icon>
        </v-btn>
        <v-btn color="error" @click="abrirEliminar">
      ...
      <v-toolbar flat color="white">
        <span class="blue--text " style="font-size: 18px">Filtrar por: </span>
        <v-spacer></v-spacer>
        <v-flex xs12 sm4 md4 ld4 xl4>
          <v-checkbox
            style="margin-top:15px"
            color="success"
            v-model="chcktodo"
        ...
        <v-flex xs12 sm12 md12 ld12 xl12></v-flex>
    
        <v-flex xs12 sm3 md3 ld3 xl3>
          <v-autocomplete
            v-model="codigoCat"
            :items="categoria"
            v-on:change="consultaxcategoria()"
            label="Categoria"
          ></v-autocomplete>
        </v-flex>
        ...
        <v-flex xs12 sm3 md3 ld3 xl3>
          <v-autocomplete
            v-model="codigoLab"
            :items="laboratorio"
            v-on:change="consultarxlaboratorio()"
            label="Laboratorio"
          ></v-autocomplete>
        </v-flex>
       
        <v-flex xs12 sm2 md2 ld2 xl2>
          <v-checkbox
            style="margin-top:15px"
            color="success"
            v-model="chckfracciones"
            label="Solo fracciones"
            @change="listarfracciones"
          ></v-checkbox>
        </v-flex>
       ...
       <v-flex xs6 sm8 md6 ld6 xl8>
          ...
          single-line
          hide-details
           v-on:keyup.enter="busquedaAvanzada()"
        ></v-text-field>
        </v-flex>
      <span class="blue--text " style="font-size: 25px">PRODUCTOS</span>
      <v-data-table
        :headers="headers"
        :items="articulos"
        :search="search"
      ...
      >
        <template v-slot:items="props">
          ...
          <td class="green--text">{{ Math.floor((props.item.fraccionesTotales/props.item.fraccionCaja)) }}</td>
          ...
          <td>{{ props.item.codigoCategoria.nombre }}</td>
          <td>{{ props.item.codigoPresentacion.descripcion }}</td>
      ...
      <span class="blue--text " style="font-size: 25px"
        >PRODUCTOS POR CADUCAR</span
      >
      <v-data-table
        :headers="headersF"
        :items="articulosF"
...
export default {
  data() {
    return {
      ...
      codigoFEx:"",
      exportarExcel:0,
      chckfraccionado:false,
      cajitas:0,
      Fr:0,
      Cajas:0,
      busquedaAavanzada:"",
      chckfracciones: false,
      chckcero: false,
      chcktodo: false,
      codigoLab: "",
      farm: [],
      ...
      headers: [
        ...
        { text: "Codigo de Barras", value: "codigoBarras", sortable: false },
        { text: "Inventario", value: "codigoInventario", sortable: false },
        { text: "Producto", value: "nombre", sortable: true },
        { text: "Principio/Nombre", value: "nombreComercial", sortable: true },
        { text: "Cajas", value: "stock", sortable: false },
        {
          text: "Fracciones",
          value: "fracciones",
          sortable: true,
        ...
        { text: "Fracciones/Caja", value: "fraccionCaja", sortable: false },
        { text: "Fecha caducidad", value: "fechaCaducidad", sortable: false },
        { text: "Costo neto", value: "costoNeto", sortable: false },
        { text: "PVP", value: "pvp", sortable: false },
        { text: "P. UNIT", value: "precioUni", sortable: false },
        { text: "Descuento", value: "descuento", sortable: true },
        { text: "IVA", value: "iva", sortable: true },
        { text: "Categoria", value: "codigoCategoria", sortable: false },
        { text: "Presentacion", value: "codigoPresentacion", sortable: false },
        { text: "Laboratorio", value: "codigoLaboratorio", sortable: false },
      ...
      descuento: 0,
      iva: "",
      ivade: [{ text: "SI", value: 1 }, { text: "NO", value: 0 }],
      codigoCategoria: "",
      categoria: [],
      codigoLaboratorio: "",
      laboratorio: [],
   ...
   verificarFraccionado(){
     if(this.chckfraccionado){
       if(this.cajitas!=0){
        ...
        this.fraccionCaja=1
        this.fraccionesTotales=1*parseInt(this.cajitas)
         this.Cajas= Math.floor(parseFloat(this.fraccionesTotales)/parseFloat(this.fraccionCaja))
        this.Fr=0
       }else{
           Swal.fire("Error","Debe ingresar el numero de cajas","error")
          this.fraccionCaja=0
   ...
   calcularCajasFr(){
    ...
    calcularCajasFracciones(){
      ...
      if(this.fraccionCaja!=1){
        this.Fr= Math.round(((parseFloat(this.fraccionesTotales)/parseFloat(this.fraccionCaja) -
         Math.floor(parseFloat(this.fraccionesTotales)/parseFloat(this.fraccionCaja))))*parseFloat(this.fraccionCaja))
      }else{
        this.Fr=0
      }
    ...
    busquedaAvanzada(){
      this.articulos=[]
      let me = this;
      let valor = this.busquedaAavanzada
      ...
      if(this.checkbox){
        ...
        if (!isNaN(valor)) {
          if (codigoFarmacia == undefined) {
            if (!this.codigoFar) {
              Swal.fire("Notificación", "Debe seleccionar una farmacia!", "error");
            }
              //Buscar el inventario a traves del codigo farmacia...
              axios
                .get("inventario/query?_id=" + this.codigoFar, configuracion) //#1
                  ...
                  axios
                    .get(
                      "productos/busquedaAvanzada?codigoInventario=" +
                        codigoInvenatio.data._id+"&codigoBarras="+valor,
                      configuracion
                    )
                    .then(function(response) {
    ...
    verifiIva(){
      ...
      // if(this.iva){
      //   let p = this.pvp
      //   this.pvp = (parseFloat(p)-(parseFloat(p)*0.12)).toFixed(2)


      // }else{
      //    this.pvp = (parseFloat(this.pvp)).toFixed(2)
       ...
       if (codigoFarmacia == undefined) {
        ...
        axios
          ...
          .then(function(codigoInvenatio) {
            ...
            axios
              .get(
                "productos/verificar?CB="+me.codigoBarras+"&codigoInventario=" +
                  codigoInvenatio.data._id,
                configuracion
              )
              .then(function(response) {
               if(response.status==200){//EXISTE
                    Swal.fire("Aviso!","El codigo de barras ingresado, YA EXISTE!","warning")
                    me.banderaexiste=true
               }else{
                 me.banderaexiste=false
               }
      ...
      if (codigoFarmacia == undefined) {
        ...
        axios
          ...
          .then(function(codigoInvenatio) {
            ...
            axios
              .get(
                "productos/duplicados?codigoInventario=" +
                  codigoInvenatio.data._id,
                configuracion
              )
              .then(function(response) {
                 ...
                 element.duplicados.forEach(element => {
                    me.articulosduplicados.push(element)
                 });
      ...
      if (codigoFarmacia == undefined) {
        ...
        axios
          ...
          .then(function(codigoInvenatio) {
            ...
            axios
              .get(
                "productos/listxcategoria?codigoCategoria=" +
                  me.codigoCat +
                  "&codigoInventario=" +
                  codigoInvenatio.data._id,
                configuracion
      ...
      if (codigoFarmacia == undefined) {
        ...
        axios
          ...
          .then(function(codigoInvenatio) {
            ...
            axios
              .get(
                "productos/listxlaboratorio?codigoLaboratorio=" +
                  me.codigoLab +
                  "&codigoInventario=" +
                  codigoInvenatio.data._id,
                configuracion
    ...
    exportarInventario() {
      ...
      let codigoFarmacia = this.$store.state.usuario.codigoFarmacia;
      let me = this;
      if(me.chckTotalizado){
        axios
              .get(
                "http://localhost:25000/api/exports/exportarTotal",
                configuracion
              )
              .then(function(response) {
                if (response.status == 200) {
                  Swal.fire(
                    "Notificación",
                    "Archivo exportado, revise la carpeta de Archivos/ArchivosExportados/INVENTARIO",
                    "success"
                  );
                } else {
                  Swal.fire("Notificación", "Ocurrió un problema..", "error");
                }
              })
              .catch(function(error) {
                console.log(error);
        ...
        if(codigoFarmacia==undefined){
        axios
          .get("inventario/query?_id=" + me.codigoFEx, configuracion1) //#1
          .then(function(codigoInvenatio) {
            //Buscar los productos que pertenecen al inventario...
            axios
              .get(
                "http://localhost:25000/api/exports/inventarioEx?codigoInventario="+codigoInvenatio.data._id+"&inventario="+codigoInvenatio.data.descripcion,
                configuracion
              )
              .then(function(response) {
                if (response.status == 200) {
    ...
    cargarInventario() {
      ...
      axios
        ...
        .then(function(codigoInvenatio) {
          ...
          axios
            .get(
              "productos/listaalfabetica?codigoInventario=" +
                codigoInvenatio.data._id +
                "&letra=" +
                me.letra,
              configuracion
            ...
            .then(function(response) {
              if (response.data.length != 0) {
                me.inventarioCargado = response.data;
                Swal.fire("Notificación", "Datos cargados", "success");
              } else {
                Swal.fire(
                  "Notificación",
                  "No hay datos con esta caracter",
                  "warning"
                );
              }
            })
    ...
    EliminarInv() {
      ...
      axios
        .delete(
          "productos/eliminar?codigoInventario=" + this.codigoInventario,
          configuracion
        )
        .then(function(response) {
          if (response.data == "OK") {
    ...
    Clonar() {
      ...
      axios
        .get(
          "productos/clonar?codigoInventario=" +
            this.codigoInventarioA +
            "&codigoNuevo=" +
            this.codigoInventarioB,
          configuracion
        )
        .then(function(response) {
          if (response.data == "OK") {
            Swal.fire(
              "Notificación",
              "Productos clonados exitosamente..",
              "success"
            );
    ...
    calcularFraccionesTotales() {
      this.fraccionesTotales =
        parseInt(this.cajitas) * parseInt(this.fraccionCaja);
    },
    calcularPrecioUnitario() {
      this.precioUni = (
        parseFloat(this.pvp) / parseFloat(this.fraccionCaja)
      ).toFixed(2);
    },
    ...
    crearPDF() {
      ...
      var columns = [
        { title: "Codigo de Barras", dataKey: "codigoBarras" },
        { title: "Inventario", dataKey: "codigoInventario" },
        { title: "Producto", dataKey: "nombreComercial" },
        { title: "PVP", dataKey: "pvp" },
        { title: "Stock/Caja", dataKey: "stock" },
        { title: "Total/Fracciones", dataKey: "fraccionesTotales" },
        { title: "Laboratorio", dataKey: "codigoLaboratorio" },
      ];
      var rows = [];
      this.articulos.map(function(x) {
        rows.push({
          codigoBarras: x.codigoBarras,
          codigoInventario: x.codigoInventario.descripcion,
          nombreComercial: x.nombreComercial,
          pvp: x.pvp,
          stock: x.stock,
          fraccionesTotales: x.fraccionesTotales,
          codigoLaboratorio: x.codigoLaboratorio.abreviatura,
        });
      });
      let me = this;
      var doc = new jsPDF();
      doc.autoTable(columns, rows, {
        ...
        addPageContent: function(data) {
          doc.text("Reporte de articulos", 80, 20);
      ...
      axios
        ...
        .then(function(response) {
          ...
          ArrayT.map(function(x) {
            me.farm.push({ text: x.descripcion, value: x._id });
          });
        })
        .catch(function(error) {
          console.log(error);
    ...
    selectPresentacion() {
      ...
      axios
        ...
        .then(function(response) {
          ArrayT = response.data;
          ArrayT.map(function(x) {
            me.presentacion.push({ text: x.descripcion, value: x._id });
          });
        })
        .catch(function(error) {
          console.log(error);
      ...
      axios
        ...
        .then(function(response) {
          ...
          LaboratorioArray.map(function(x) {
            me.laboratorio.push({ text: x.nombre, value: x._id });
          });
        })
        .catch(function(error) {
          console.log(error);
    ...
    listarF() {
      ...
      if (codigoFarmacia == undefined) {
        axios
          .get("productos/listtotal", configuracion)
          .then(function(response) {
    ...
    validar() {
      this.valida = 0;
      this.validaMensaje = [];
      if (!this.codigoLaboratorio) {
        this.validaMensaje.push("Selecciona una laboratorio.");
      }
      if (!this.codigoCategoria) {
        this.validaMensaje.push("Selecciona una categoria.");
      }
      if (!this.codigoPresentacion) {
        this.validaMensaje.push("Selecciona una presentacion.");
      }
      if (!this.codigoInventario && this.chcktodoi==false) {
        this.validaMensaje.push("Selecciona un inventario.");
      }
      if (!this.fechaCaducidad) {
        this.validaMensaje.push("Debe ingresar la fecha de caducidad");
      }
      ...
      if(this.fraccionCaja==0){
          this.validaMensaje.push("El fraccionamiento de la caja no puede ser 0.")
      }
      if (this.descuento>=0) {
      ...
      if (!this.nombreComercial) {
        this.validaMensaje.push("Ingrese el nombre comercial.");
      }
      if (!this.codigoBarras) {
        this.validaMensaje.push("Ingrese el codigo de barras.");
      }
      if (!this.pvp) {
    ...
    calucularFraccionamiento(fraccionesTotales,fraccionCaja){
      ...
      if(fraccionCaja!=1){
        this.Fr= Math.round(((parseFloat(fraccionesTotales)/parseFloat(fraccionCaja) -
         Math.floor(parseFloat(fraccionesTotales)/parseFloat(fraccionCaja))))*parseFloat(fraccionCaja))
      }else{
        this.Fr=0
      }
    },
    ...
    //  console.log(item);
      ...
      this.calucularFraccionamiento(item.fraccionesTotales,item.fraccionCaja)
      ...
      this.codigoCategoria = item.codigoCategoria._id;
      this.codigoLaboratorio = item.codigoLaboratorio._id;
      this.codigoPresentacion = item.codigoPresentacion._id;
      this.codigoInventario = item.codigoInventario._id;
      this.pvp = item.pvp;
      this.precioUni = item.precioUni;
    ...
    activar() {
      let me = this;
      axios
        .put("productos/activate", { _id: this.adId })
        .then(function(response) {
          me.adModal = 0;
          me.adAccion = 0;
          me.adNombre = "";
    ...
    desactivar() {
      let me = this;
      axios
        .put("productos/deactivate", { _id: this.adId })
        .then(function(response) {
          me.adModal = 0;
          me.adAccion = 0;
          me.adNombre = "";
    ...
    listar() {
      ...
      if (this.chcktodo == true) {
        if (codigoFarmacia == undefined) {
          ...
          axios
            .get("inventario/query?_id=" + this.codigoFar, configuracion1) //#1
            .then(function(codigoInvenatio) {
              //Buscar los productos que pertenecen al inventario...
              axios
                .get(
    ...
    listarcero() {
      ...
      if (this.chckcero == true) {
        if (codigoFarmacia == undefined) {
          ...
          axios
            .get("inventario/query?_id=" + me.codigoFar, configuracion1) //#1
            .then(function(codigoInvenatio) {
              //Buscar los productos que pertenecen al inventario...
              axios
                .get(
                  "productos/listcero?valor=" + codigoInvenatio.data._id,
                  configuracion
                )
                .then(function(response) {
                  me.articulos = response.data;
        ...
        if (codigoFarmacia == undefined) {
          ...
          axios
            ...
            .then(function(codigoInvenatio) {
              ...
              axios
                .get(
                  "productos/listfracciones?valor=" + codigoInvenatio.data._id,
                  configuracion
                )
                .then(function(response) {
                  me.articulos = response.data;
    ...
    encabezadosXML(usuario) {
      ...
      axios
        .post(
          "http://localhost:25000/api/xml/xmlInventario",
          {
            inventario: {
              usuario: usuario,
            },
            detalles: {
              detalle: this.inventarioCargado,
            },
          },
          configuracion
        )
    ...
    imprimirAlfabetico() {
      if (this.inventarioCargado.length != 0) {
        this.encabezadosXML("DEPENDIENTE");
      } else {
        Swal.fire(
          "Notificación",
          "Debe dar click en consultar primero...",
          "error"
        );
      }
    },
