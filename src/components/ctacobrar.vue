    ...
    <v-flex>
      <v-toolbar flat color="white">
                ...
                <v-layout wrap>
                  ...
                  <v-flex xs3 sm3 md3 xl3>
                    <v-checkbox
                      style="margin-top: 15px"
                      color="success"
                      v-model="chckFarmacia"
                      label="Ver reporte por farmacia."
                    ></v-checkbox>
                  </v-flex>
                  <v-flex xs12 sm12 md12 xl12 v-if="chckCliente">
                    <v-autocomplete
                      v-model="codigoPersona"
                      :items="personas"
                      label="Clientes"
                    ></v-autocomplete>
                  </v-flex>
                  <v-flex xs12 sm12 md12 xl12 v-if="chckFarmacia">
                    <v-autocomplete
        ...
        <v-dialog v-model="adModal" max-width="290">
          <v-card>
            ...
            <v-card-text>
              Estás a punto de <span v-if="adAccion == 1">canlear </span>
              <span v-if="adAccion == 2">Pagar </span> la cuenta por cobrar de
              la factura #{{ adNombre }}
            </v-card-text>
            <v-card-actions>
              <v-spacer></v-spacer>
      ...
      <v-expansion-panel>
        <v-expansion-panel-content>
          ...
          <template v-slot:header>
            <div class="orange--text font-weight-bold">
              Filtros <v-icon>arrow_circle_right</v-icon>
            </div>
          </template>
          <v-card>
            <v-container grid-list-md>
              <v-layout wrap>
                ...
                <v-flex xs4 sm4 md4>
                  <v-autocomplete
                      ...
                      :items="estadosFilters"
                      label="Estado"
                    ></v-autocomplete>
                </v-flex>
                
                <v-flex xs12 sm12 md12 xl12></v-flex>
                <v-flex xs12 sm6 md6>
                  <v-dialog
                    ref="dialog5"
                    v-model="modal5"
                    :return-value.sync="finicio5"
                    persistent
                    width="290px"
                  >
                    <template v-slot:activator="{ on, attrs }">
                    ...
                    <v-date-picker v-model="finicio5" scrollable>
                      <v-spacer></v-spacer>
                      ...
                      <v-btn
                        text
                        color="success"
                        @click="$refs.dialog5.save(finicio5)"
                        >Aceptar</v-btn
                      >
                    </v-date-picker>
                  </v-dialog>
                ...
                <v-flex xs12 sm6 md6>
                  ...
                  >
                    ...
                    <v-date-picker v-model="ffin6" scrollable>
                      <v-spacer></v-spacer>
                      <v-btn text color="primary" @click="modal6 = false"
                        >Cancelar</v-btn
                      >
                      <v-btn
                        text
                        color="success"
                        @click="$refs.dialog6.save(ffin6)"
                        >Aceptar</v-btn
                      >
                    </v-date-picker>
                  </v-dialog>
            ...
            <v-btn color="white" class="info" flat small @click="listarConFiltro()">
      ...
      >
        <template v-slot:items="props">
          ...
          <td v-if="props.item.codigoPersona">
            {{ props.item.codigoPersona.nombres }}
          </td>
          <td v-else>undefined</td>
          <td>{{ props.item.codigoFarmacia.descripcion }}</td>
          <td>
            <div v-if="props.item.estado">
              <span class="red--text">Por pagar</span>
            </div>
            <div v-else>
      ...
      <v-data-table
        :headers="headerstotalizado"
        :items="totalizado"
        :search="search"
        class="elevation-1"
        v-if="chckTotalizado == true"
      >
        <template v-slot:items="props">
          <td v-if="props.item._id.codigoPersona">
            {{ props.item._id.codigoPersona.nombres }}
          </td>
          <td v-else>undefined</td>
...
export default {
  data() {
    return {
      ...
      headerstotalizado: [
        {
          text: "Cliente",
          value: "_id.codigoPersona.nombres",
          sorteable: true,
        },
      ...
      headers: [
        ...
        { text: "Fecha", value: "craetedAt", sorteable: true },
        { text: "Factura", value: "descripcion", sortable: false },
        ...
        { text: "Cliente", value: "codigoPersona.nombres", sortable: false },
      ...
      items: [],
      estadoFilter:"",
      estadosFilters:[{text:"Pagado",value:"Pagado"},{text:"No Pagado",value:"No Pagado"}],
      editedIndex: -1,
      _id: "",
      descripcion: "",
      valida: 0,
  ...
  methods: {
    listarConFiltro(){
      ...
      let estadoF=0;
      if(me.estadoFilter==="Pagado"){
        estadoF=0;
      }else{
        estadoF=1
      }
      axios
        .post("ctascobrar/listxfiltro",{filtro:me.select,finicio:me.finicio5,ffin:me.ffin6,estado:estadoF}, configuracion)
    ...
    filtros(){
      ...
      let configuracion = { headers: header };
      axios
        .get("cupo/filtro", configuracion)
        .then(function (response) {
          personaArray = response.data;

          personaArray.map(function (x) {
            me.items.push({ text: x.descripcion, value: x.descripcion });
          });
        })
        .catch(function (error) {
          console.log(error);
    ...
    listar() {
      ...
      let configuracion = { headers: header };
      axios
        .get("ctascobrar/list", configuracion)
        .then(function (response) {
          me.rol = response.data;
        })
        .catch(function (error) {
    ...
    validar() {
      ...
      if (this.descripcion.length > 255) {
        this.validaMensaje.push(
          "La descripción  no debe tener más de 255 caracteres."
        );
      }
    ...
    guardar() {
      ...
      if (this.editedIndex > -1) {
        //Código para editar
        axios
          .put("rol/update", { _id: this._id, descripcion: this.descripcion })
          .then(function (response) {
            me.limpiar();
            me.close();
            me.listar();
        ...
        //Código para guardar
        axios
          .post("rol/add", { descripcion: this.descripcion })
          .then(function (response) {
            me.limpiar();
            me.close();
            me.listar();
    ...
    activarDesactivarMostrar(accion, item) {
      this.totalcuenta = item.total;
      this.idPersona = item.codigoPersona._id;
      this.adModal = 1;
    ...
    activar() {
      let me = this;
      let cupomin, idcupo, cupototal;
      let header = { Token: this.$store.state.token };
      let configuracion = { headers: header };
      axios
        .get("cupo/existe?codigoPersona=" + this.idPersona, configuracion)
        .then(function (response) {
          if (response.status == 200) {
            ...
            cupototal = parseFloat(cupomin) + parseFloat(me.totalcuenta);
            me.actualizarCupo(idcupo, cupototal);
          }
        })
        .catch(function (error) {
          console.log(error);
      ...
      axios
        .put("ctascobrar/activate", { _id: this.adId })
        .then(function (response) {
          me.adModal = 0;
          me.adAccion = 0;
          me.adNombre = "";
    ...
    desactivar() {
      ...
      axios
        .put("ctascobrar/deactivate", { _id: this.adId })
        .then(function (response) {
          me.adModal = 0;
          me.adAccion = 0;
          me.adNombre = "";
    ...
    filtrarFechas() {
      ...
      let fechaConverI = new Date(fechaInicial + " 00:00:00");
      ...
      if (me.chckCliente) {
        axios
          .get(
            "ctascobrar/listporcliente?fechainicio=" +
          ...
          .then(function (response) {
            me.rol = response.data;
            me.ocultarExportar();
            me.chckCliente = false;
            me.codigoPersona = "";
          })
          .catch(function (error) {
            console.log(error);
          });
      } else if (me.chckTotalizado) {
        axios
          .get(
            "ctascobrar/reporteCuentas?start=" +
              fechaConverI.toISOString() +
              "&end=" +
              fechaConverF.toISOString()
          )
          .then(function (response) {
            me.totalizado = response.data;
            me.ocultarExportar();
          })
          .catch(function (error) {
            console.log(error);
          });
      } else if (me.chckFarmacia) {
        axios
          .get(
            "ctascobrar/listporfechas?fechainicio=" +
              fechaConverI.toISOString() +
              "&fechafin=" +
              fechaConverF.toISOString() +
              "&codigoFarmacia=" +
    ...
    crearPDF() {
      ...
      if (me.chckTotalizado) {
        columns = [
          { title: "Cliente", dataKey: "codigoPersona" },
          { title: "Total", dataKey: "total" },
        ];
        let nombre = "";

        this.totalizado.map(function (x) {
          // console.log(x);
          if (x._id.codigoPersona) {
            nombre = x._id.codigoPersona.nombres;
          } else {
            nombre = "undefined";
          }
          rows.push({
            total: x.total,
            codigoPersona: nombre,
          });
        });
      } else {
        columns = [
          { title: "Numero de comprobante", dataKey: "numComprobante" },
          { title: "Detalle", dataKey: "descripcion" },
        ...
        this.rol.map(function (x) {
          ...
          let nombre = "";
          if (x.codigoPersona) {
            nombre = x.codigoPersona.nombres;
          } else {
            nombre = undefined;
          }
      ...
      doc.autoTable(columns, rows, {
        ...
        addPageContent: function (data) {
          ...
          doc.text(
            "Reporte de créditos del mes de " + month.toUpperCase(),
            60,
            40
          );
      ...
      doc.save("ReporteCrediticio.pdf");
    },
  },
};
</script>
